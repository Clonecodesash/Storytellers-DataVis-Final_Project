...
import { MapDensityComponent } from '../map-density/map-density.component';

@Component({
  selector: 'app-maps',
  standalone: true,
  imports: [RouterModule, CommonModule, FormsModule, MapDensityComponent],
...
export class MapsComponent {
  ...
  private createColorScale(mergedData: any): any {
    return d3.scaleSequential(d3.interpolateReds)
      .domain([0, d3.max(mergedData.features, (d: any) => parseFloat(d.properties.Population_Emission)) || 0]); // Color scale from white to dark red
  }
  ...
  private drawMap(svg: any, mergedData: any, colorScale: any, path: any): void {
    ...
    svg
      ...
      .join('path')
      .attr('d', path)
      .attr('fill', (d: any) => colorScale(d.properties.Population_Emission))
      .attr('stroke', 'white')
